    ...
    <article>
      ...
      <DocsSection id="tailwind" title="Tailwind setup" className="mt-10">
        <p className="text-sm text-muted-foreground">
          Adapter components use Tailwind utility classes. Add the package to
          your Tailwind content scan:
        </p>
        <CopyCodeBlock>{`/* app/globals.css */
@source "../../../node_modules/@formura/adapters/dist/**/*.{js,mjs}";`}</CopyCodeBlock>
        <p className="text-sm text-muted-foreground">
          Monorepo? Point at source instead:
        </p>
...
const { Form } = createForm({ schema, adapter, action });`}</CopyCodeBlock>
        <p className="text-sm text-muted-foreground">
          Extend{" "}
          <code className="text-foreground">createShadcnAdapter</code> in your
          app to override individual widget renderers or the field wrapper.
        </p>
      </DocsSection>
...
// name, value, onChange, onBlur, disabled, error, label, meta, fieldSchema`}</CopyCodeBlock>
        <p className="text-sm text-muted-foreground">
          ...
          <code className="text-foreground">inferFieldMeta</code> to decide
          which component to render. See the{" "}
