...
- Removed `next` peer dependency (Formura does not import Next.js)
- Moved `zod` from dependencies to peerDependencies (`^4.0.0`)

## [0.0.3] - 2026-07-08

### Added

- `date`, `time`, and `datetime` widgets in `FieldWidget`
- Date field inference from `z.date()` and `z.coerce.date()`
- Time field inference from `z.string().describe("widget:time")`
- Date-time field inference from `z.coerce.date().describe("widget:datetime")`
- Tests for date, time, and date-time widget inference
